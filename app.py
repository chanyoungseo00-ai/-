...
# 💡 [핵심] 이미지 파일이 없어도 에러가 나지 않도록 로직 변경
def display_logo():
    st.title("🏑 그라운드골프 대진표 편성 시스템")
    st.write("---")

try:
    # ( assign_teams_and_orders 및 create_print_excel 함수는 이전과 동일함 )
    # 생략된 함수들은 이전 코드 그대로 사용하시면 됩니다.
    ...
    def assign_teams_and_orders(df, holes_per_field=8, p_cnt_indiv=6, p_cnt_team=6, match_type="개인전"):
        ...
        for idx, team in enumerate(teams):
            ...
            r_name = f"{round_id}부"
            for p in team:
                final_roster.append({'경기': r_name, '구장': field_name, '홀': hole, '팀': f"{idx+1}조", '타순': p['타순'], '대진표': f"{field_name} {hole} {p['타순']}", '부문': p.get('부문', match_type), '지역': p['지역'], '이름': p['이름'], '성별': p['성별'], '_r': round_id, '_f': f_idx, '_h': hole})
        return pd.DataFrame(final_roster).sort_values(by=['_r', '_f', '_h', '타순']).reset_index(drop=True).drop(columns=['_r', '_f', '_h']), num_teams
            ...
            for r_name in sorted(df['경기'].unique()):
                ...
                for f_name in ['청', '백', '홍', '황']:
                    ...
                    for h in range(1, holes_cnt + 1, 2):
                        ...
                        for c, text in enumerate(heads): ws.write(row, c, text, head_fmt); ws.write(row, c+8, text, head_fmt)
                        row += 1
                        for i in range(max(len(h1_data), len(h2_data), 6)):
                            if i < len(h1_data): p = h1_data.iloc[i]; ws.write_row(row+i, 0, [h if i==0 else "", p['타순'], p['부문'], p['지역'], p['이름'], p['성별'], ""], cell_fmt)
                            if i < len(h2_data): p = h2_data.iloc[i]; ws.write_row(row+i, 8, [h+1 if i==0 else "", p['타순'], p['부문'], p['지역'], p['이름'], p['성별'], ""], cell_fmt)
    ...
    def process_raw_data(df_raw, default_category):
        ...
        df_raw.rename(columns={'성명':'이름', '선수명':'이름', '소속':'지역', '시군구':'지역', '클럽':'지역', '남여':'성별'}, inplace=True)
        for col in ['지역', '이름']: df_raw[col] = df_raw[col].astype(str).str.strip().replace(r'^\s*$', np.nan, regex=True).replace(['nan', 'None', 'NaN'], np.nan)
    ...
    def load_gsheet(url):
        if not url or "/d/" not in url: return None
    ...
    # [데이터 로드 로직 생략(위 코드와 동일)]
    # (위의 코드에서 데이터 로드 부분 복사해서 사용하세요)
    # 생략된 데이터 처리 로직은 위의 코드와 100% 동일합니다.
